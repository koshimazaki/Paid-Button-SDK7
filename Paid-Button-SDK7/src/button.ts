...
const openSound = engine.addEntity()
const closeSound = engine.addEntity()
...
addAudio(openSound,'sounds/LaserHum.mp3')
addAudio(closeSound,'sounds/access_denied.mp3')
...
// animations 
  ...
  // Scanner 
  export const Button = engine.addEntity()
  GltfContainer.create(Button, { src: 'models/PaidButton/MANA_Button.glb' }) 
  MeshRenderer.create(Button)
...
function updateScannerSoundAndAnimation() {
  ...
  const scannerAnimator = Animator.getMutable(Button)
  if (scannerAnimator) {
...
function getPlayerPosition() {
    ...
    const scannerPos = Transform.get(Button).position;
...
Animator.getMutable(Button).states.forEach(state => state.playing = false)
